...
from .config import ColumnConfig, GridConfig, SelectionMode
from .theme import ensure_aggrid_theme
...
class CustomAgGrid:
    ...
    Signals:
        cell_edited (int, str, object, object, dict):
            Emitted when a cell is edited. Arguments are
            ``(row_index, field_name, old_value, new_value, full_row_dict)``.
        row_selected (int, dict):
            Emitted when a row is selected. Arguments are
            ``(row_index, full_row_dict)``.
    """

    # Signals are class attributes; psygnal handles per-instance connections.
    cell_edited: Signal = Signal(int, str, object, object, dict)
    row_selected: Signal = Signal(int, dict)
    ...
    def to_pandas(self) -> "pd.DataFrame":
        ...
        """
        if not HAS_PANDAS:
            raise ImportError("pandas is not available. Install 'pandas' to use to_pandas().")
    ...
    def to_polars(self) -> "pl.DataFrame":
        ...
        """
        if not HAS_POLARS:
            raise ImportError("Polars is not available. Install 'polars' to use to_polars().")
    ...
    def _on_cell_value_changed(self, e: events.GenericEventArguments) -> None:
        ...
        This updates the internal row representation and emits the
        :attr:`cell_edited` signal.
        ...
        if row_index is None or field is None:
            ...
            # print("DEBUG cellValueChanged args:", args)
            return
        ...
        self.cell_edited.emit(i, field, old_value, new_value, row_data)

    def _on_row_selected(self, e: events.GenericEventArguments) -> None:
        """Handle AG Grid ``rowSelected`` events from NiceGUI.

        This tracks the last known selection and emits the
        :attr:`row_selected` signal when a row becomes selected.
        ...
        self.row_selected.emit(i, row_data)
