...
def _format_swarm_columnar_to_tsv(summary: PlotSummary) -> str:
    ...
    for key, sub in groups:
        ...
        points = list(zip(file_stems, x_jitters, ys, strict=False))
    ...
    for group_val, color_val, _points in sets_data:
    ...
    for r in range(max_rows):
        row_cells: list[str] = []
        for _group_val, _color_val, points in sets_data:
            if r < len(points):
                fs, xj, yv = points[r]
                if has_x_jitter:
...
def _format_scatter_columnar_to_tsv(summary: PlotSummary) -> str:
    ...
    for key, sub in groups:
        ...
        points = list(zip(
            ...
            sub["file_stem"].astype(str).tolist(), strict=False,
        ))
        sets_data.append((group_val, points))
    max_rows = max(len(s[1]) for s in sets_data)
    for group_val, _points in sets_data:
        row0_cells.append(group_val)
        row0_cells.extend([""] * (block_width - 1))
        row0_cells.append("")
    ...
    for r in range(max_rows):
        row_cells: list[str] = []
        for _group_val, points in sets_data:
            if r < len(points):
                xv, yv, fs = points[r]
...
def _format_hist_columnar_to_tsv(summary: PlotSummary, value_col: str) -> str:
    ...
    for key, sub in col.groupby(group_col_name, sort=True):
        ...
        points = list(zip(
            ...
            sub[value_col].tolist(), strict=False,
        ))
    ...
    for group_val, _points in groups:
        row0_cells.append(group_val)
        row0_cells.extend([""] * (block_width - 1))
        row0_cells.append("")
    ...
    for r in range(max_rows):
        row_cells: list[str] = []
        for _group_val, points in groups:
            if r < len(points):
                bc, v = points[r]
