...
from nicewidgets.nicepool.pre_filter_conventions import (
    PRE_FILTER_NONE,
    default_pre_filter,
...
class PlotPoolConfig:
    ...
    Attributes:
        ...
        initial_plot_config: Optional inline plot configuration dict with the same shape as a
            ...
            ``control_panel_splitter_value``. Host applications use this for
...
class PlotPoolController:
    ...
    **Public API (entry point for external users):**
    ...
    def __init__(
        ...
        df: pd.DataFrame,
        *,
        config: PlotPoolConfig | None = None,
    ) -> None:
        ...
        # before a host application has loaded analysis-pool rows.
        x_default, y_default = self._default_plot_columns(df)
        ...
        self._clicked_label: ui.label | None = None
        self._mainSplitter: ui.splitter | None = None
        self._verticalSplitter: ui.splitter | None = None
        self._last_plot_type: PlotType | None = None
        self._plot_container: ui.column | None = None
        self._selection_label: ui.label | None = None
        self._control_panel: PoolControlPanel | None = None
        self._table_view: DataFrameTableView | None = None
        ...
        self._plot_summaries: list[PlotSummary | None] = [None] * 4
    ...
    def update_df(self, new_df: pd.DataFrame) -> None:
        ...
        self.plot_states = [self._validate_plot_state_columns(state) for state in self.plot_states]
        self.default_plot_state = self._validate_plot_state_columns(self.default_plot_state)
    ...
    def build(self, *, container: ui.element | None = None) -> None:
    ...
    def _debounced_save_splitter(self) -> None:
        ...
        if not self._enable_config_persistence:
            return
        if self._splitter_save_timer is not None:
            self._splitter_save_timer.cancel()
    ...
    def _default_plot_columns(self, df: pd.DataFrame) -> tuple[str, str]:
        """Return safe default x/y columns for a DataFrame.

        Numeric columns are preferred because most plot types use numeric y
        values. When no numeric columns are present, fall back to ordinary
        columns so NicePool can build against empty/schema-only DataFrames and
        later receive real data via update_df()/set_dataframe().

        Args:
            df: DataFrame used by the pool.

        Returns:
            Tuple of (x column, y column).

        Raises:
            ValueError: If the DataFrame has no columns.
        """
        all_cols = [str(column) for column in df.columns]
        if not all_cols:
        ...
        x_default = all_cols[0]
        y_default = all_cols[1] if len(all_cols) >= 2 else all_cols[0]
        return x_default, y_default
    ...
    def _save_config(self) -> None:
        ...
        if not self._enable_config_persistence:
            ui.notify("Plot configuration persistence is disabled", type="warning")
            return
        from nicewidgets.nicepool.pool_plot_config import PoolPlotConfig
    ...
    def _on_plotly_click(self, e: GenericEventArguments, plot_index: int = 0) -> None:
        ...
        if plot_state.plot_type in {PlotType.SCATTER, PlotType.SWARM}:
            ...
            if self._clicked_label:
                self._clicked_label.text = (
                    f"Plot {plot_index + 1}: {format_pre_filter_display(plot_state.pre_filter)} "
        ...
        if plot_state.plot_type == PlotType.GROUPED:
            ...
            if self._clicked_label:
                self._clicked_label.text = (
                    f"Plot {plot_index + 1}: {format_pre_filter_display(plot_state_dict.pre_filter)} "
                    f"clicked group={x}, y={y} (aggregated)"
                )
            return
