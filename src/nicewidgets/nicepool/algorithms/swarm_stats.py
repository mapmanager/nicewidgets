...
def swarm_full_stats_table(
    ...
    *,
    ...
    if use_color:
        tmp["_group_key"] = list(zip(tmp["x"].astype(str), tmp["color"].astype(str), strict=False))
    else:
        tmp["_group_key"] = list(zip(tmp["x"].astype(str), [None] * len(tmp), strict=False))
